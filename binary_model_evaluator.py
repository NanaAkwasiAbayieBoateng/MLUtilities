...
def Evaluate(labels, predictions, p=0.5):
    ...
    ks_statistic = stats.ks_2samp(labels, predictions)
    gini  =  2*auc -1
    print(f" ks statistic {ks_statistic}")
    return {'TN':TN,'FN':FN,'TP':TP,'FP':FP,'AUC':auc,'precision':prec,'recall':rec,'f1':f1,'PRAUC':area,'average_precision':average_precision,
    'kappa':kappa,'balanced_accuracy':balanced_accuracy,'ks statistic':ks_statistic, 'gini coefficient':gini}
